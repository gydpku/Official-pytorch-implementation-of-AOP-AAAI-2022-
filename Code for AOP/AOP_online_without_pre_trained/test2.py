...
parser.add_argument('--nepochs', default=5, type=int, required=False, help='(default=%(default)d)')
...
os.environ["CUDA_VISIBLE_DEVICES"] = "6"  # use gpu0,1
...
import AOP_mnist as approach
import mnist_model_mnist as network

########################################################################################################################

# Load
print('Load data...')
data, taskcla, inputsize = dataloader.get_MNIST_2(seed=args.seed)
...
appr = approach.Appr(model=net, nepochs=args.nepochs, lr=args.lr, args=args)
...
for t, ncla in taskcla:
    ...
    print("AAA",t)
    appr.train(t, xtrain, ytrain, xvalid, yvalid, data)#,lr)
    print('-'*100)
