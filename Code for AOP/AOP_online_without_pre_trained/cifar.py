...
def get_MNIST(seed=0,pc_valid=0.10):
    ...
    return data, taskcla[:10//data[0]['ncla']], size
def get_MNIST_2(seed=0,pc_valid=0.10):
    data = {}
    taskcla = []
    size = [1, 28, 28]
    # CIFAR10
    if not os.path.isdir('/home/guoyd/Dataset/MNIST2'):
        os.makedirs('/home/guoyd/Dataset/MNIST2')
        t_num = 2
        mean = [x / 255 for x in [0.1307, ]]
        std = [x / 255 for x in [0.3081,]]
        dat={}
        ...
        for t in data.keys():
            for s in ['train', 'test']:
                ...
                data[t][s]['y'] = torch.LongTensor(np.array(data[t][s]['y'], dtype=int)).view(-1)
                torch.save(data[t][s]['x'],
                           os.path.join(os.path.expanduser('/home/guoyd/Dataset/MNIST2'), 'data' + str(t) + s + 'x.bin'))
                torch.save(data[t][s]['y'],
                           os.path.join(os.path.expanduser('/home/guoyd/Dataset/MNIST2'), 'data' + str(t) + s + 'y.bin'))
    ...
    for i in range(6):
        ...
        for s in ['train','test']:
            data[i][s]={'x':[],'y':[]}
            data[i][s]['x']=torch.load(os.path.join(os.path.expanduser('/home/guoyd/Dataset/MNIST2'),'data'+str(ids[i])+s+'x.bin'))
            data[i][s]['y']=torch.load(os.path.join(os.path.expanduser('/home/guoyd/Dataset/MNIST2'),'data'+str(ids[i])+s+'y.bin'))
        data[i]['ncla'] = len(np.unique(data[i]['train']['y'].numpy()))
        data[i]['name'] = 'MNIST->>>' + str(i * data[i]['ncla']) + '-' + str(data[i]['ncla'] * (i + 1) - 1)
